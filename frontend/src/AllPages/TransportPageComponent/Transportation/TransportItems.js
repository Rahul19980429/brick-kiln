...
const TransportItems = (props) => {
    ...
    const { itemInput, setItemInput, transportFuel, saleBill } = initalvalues;
    ...
    const ClickSuggestion =(data)=>{
            setItemInput({...itemInput, Quantity:data.quantity,RefNo:data.refNo, Rate:'' , Other: '', Amount: 0})
            setItemName({...itemName, iname:data.item ,category:'sale'})
    }
            ...
            <div className='row'>
                ...
                <div className='col-lg-3 table-responsive d-none d-md-block' style={{ height: '45vh' }}>
                    ...
                    <table className="table table-success table-striped mb-0" >
                        <tbody>
                            {saleBill.length > 0 ?
                                saleBill.map((data) => data.itemsArray.map((item, index) => {
                                    return <tr style={{cursor:'pointer'}} key={index} onClick={()=>ClickSuggestion(item)} >
                                        <td>{item.item}</td>
                                        <td>{item.quantity}</td>
                                        <td>{item.refNo}</td>
                                    </tr>
                                ...
                                : <tr><td colSpan={4} className='text-center'>No Suggestion</td></tr>}
                        </tbody>
                    </table>
