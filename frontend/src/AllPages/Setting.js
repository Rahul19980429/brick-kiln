...
const Setting = () => {
    ...
    const { GetSingleUser, error, setError, logOutClick } = a;
    const [user, setUser] = useState('')
    const [Input, setInput] = useState({_id:'', name: '', contact: '', address: '', password: '' });
    ...
    const submit = (e) => {
        ...
        if (!Input.name || !Input.contact || !Input.address || !Input.password) {
            alert("Empty field is not allowed")
        }
        else {
            // AddNew(Input.itemName.toLowerCase(), itemInput.category, itemInput.itemRate);
            clearInput()
    ...
    // }
    const clearInput = () => {
        setInput({_id:'', name: '', contact: '', address: '', password: '' })

    
    }
    const editUser = (userdata) => {
        setInput({_id:userdata._id, name: userdata.name, contact: userdata.contact, address: userdata.address, password: '' })
            ...
            <div className='container py-3'>
                <div className='row px-3'>
                    <div className='col-lg-12 col-12'>
                        <h3 className='text-center bg-dark text-white mb-0 py-2'>Setting</h3>
                        <h4 className='mb-5 mt-4 text-center text-danger'>Here You Can Make Changes In Your Account.</h4>
                    </div>
                    <div className='col-lg-6 '>
                        ...
                        <fieldset id='UserForm' disabled={Input._id===''?'disabled':''}>
                            ...
                            <div className="input-group mb-3 border border-white rounded-1">
                                ...
                                <input autoComplete='off' type="text" className="bg-dark text-danger border-0 form-control" placeholder='Enter New Password' onChange={onChange} value={Input.password} id="password" name="password" aria-describedby="emailHelp" />
                            </div>
                            ...
                            <div className="d-flex gap-2 mt-2 justify-content-center">
                                <button className="btn btn-danger text-white fw-bold  mb-4 border-0 " type="submit" disabled={!Input._id || !Input.name || !Input.contact || !Input.address || !Input.password ? true : false}>SAVE CHANGES</button>
                                <button className="btn btn-danger text-white fw-bold  mb-4 border-0 " onClick={() => clearInput()} type='button' disabled={!Input.name && !Input.contact && !Input.address && !Input.password ? true : false} >CANCEL</button>
                            </div>
                            </fieldset>
                        </form>
                ...
                <div className='row px-3'>
                    <div className='col-lg-12 d-flex'>
                        <button onClick={() => alert('btn')} className="btn btn-sm btn-danger text-white fw-bold  mx-2 border-0 rounded-2" type='button'>RESET PURCHASE BILL</button>
                        <button onClick={() => alert('btn')} className="btn btn-sm btn-danger text-white fw-bold  mx-2 border-0 rounded-2" type='button'>RESET SALE BILL</button>
                        <button onClick={() => alert('btn')} className="btn btn-sm btn-danger text-white fw-bold  mx-2 border-0 rounded-2" type='button'>RESET TRANSPORT BILL</button>
                        <button onClick={() => alert('btn')} className="btn btn-sm btn-danger text-white fw-bold  mx-2 border-0 rounded-2" type='button'>RESET LABOR BILL</button>
                        <button onClick={() => alert('btn')} className="btn btn-sm btn-danger text-white fw-bold  mx-2 border-0 rounded-2" type='button'>RESET MEMBER</button>
                        <button onClick={() => alert('btn')} className="btn btn-sm btn-danger text-white fw-bold  mx-2 border-0 rounded-2" type='button'>RESET ITEM</button>
                        <button onClick={() => alert('btn')} className="btn btn-sm btn-danger text-white fw-bold  mx-2 border-0 rounded-2" type='button'>RESET COMPLETE APPLICATION</button>
