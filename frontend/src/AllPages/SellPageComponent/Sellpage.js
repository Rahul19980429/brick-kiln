...
const Sellpage = () => {
  ...
  const { recAmount, setRecAmount, payAmount, setPayAmount, discount, setDiscount,
    getAllSellBill, customerItems, setCustomerItems, selectedCustomer
    , setSelectedCustomer, sellBill, setSellBill, ADDNewSellBill, members,
    billNumberForNextBtn, setItemName, setFinalAmount, DeleteSaleBill, setError, logOutClick, finalAmount, spinner } = a;
      ...
      <div className='container mt-2'>
        ...
        <div className='row my-1'>
          ...
          <div className='col-lg-5 col-12 text-center py-1'>
            <Link to="/member" className='btn btn-primary btn-sm mx-1'>Create New Member</Link>
            <Link to="/item" className='btn btn-primary  btn-sm mx-1'>Create New Item</Link>
          </div>
          <div className='col-lg-3 col-12'>
            <div className="input-group input-group-sm mb-2 border border-white rounded-1">
              <button className='btn btn-primary'> <span className="py-0 input-group-text  border-0 rounded-0 bg-primary text-white rounded-start" id="basic-addon1">Bill No.</span></button>
          ...
          <div className='col-lg-6'>
            <div className="row g-3">
              <div className='d-flex '>
                ...
                <div className='col-9'>
                  <div className='d-flex'>
                    <button type="button" className="btn btn-primary me-3 btn-sm" id="HiddenBtnCustomer" data-bs-toggle="modal" data-bs-target="#staticBackdrop3">
                      Customer
                    </button>
        ...
        <CustomerItems btnColor={'primary'} initalvalues={{ itemInput, setItemInput }} />
        {/* part forth rec,pay,discount and other functionality (all buttons) */}
        <div className='row mt-2'>
          <div className='col-lg-5'>
            <div className='d-flex' style={{ flexWrap: 'wrap' }}>
              <ReceiptCashpage btnColor={'primary'} />
              <PaymentCashpage btnColor={'primary'} />
              <Discountpage btnColor={'primary'} />

            </div>
            <hr className='mt-2 mb-1' />
            <div className='d-flex' style={{ flexWrap: 'wrap' }}>
              <button className='col me-2 btn btn-primary mt-2' onClick={() => clearAll()}>Cancel</button>
              <button className='col me-2 btn btn-primary mt-2' id="preBtn" disabled={sellBill.SellBillNumber === 1 ? true : false} onClick={() => previousBtnClick()}>Prev </button>
              <button className='col me-2 btn btn-primary mt-2' disabled={billNumberForNextBtn < sellBill.SellBillNumber + 2 ? true : false} onClick={() => nextBtnClick()}>Next</button>
              <button className='col me-2 btn btn-primary mt-2' onClick={() => NewbtnClick()}>New</button>
              <button className='col me-2 btn btn-primary mt-2' disabled={billId ? false : true} onClick={() => DeleteBill(billId)}>Delete</button>
              <button className="col me-2 btn btn-primary mt-2" disabled={billId ? false : true} id="HiddenBtnPrint" data-bs-toggle="modal" data-bs-target="#staticBackdrop10">Print</button>
              <button className='col me-2 btn btn-primary mt-2' onClick={billSaveBtn}>Save</button>
            </div>
            <hr className='d-lg-none' />
          </div>
          {/* billing table part */}
